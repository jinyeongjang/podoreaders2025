...
export default function FamilyRegistrationUiwanPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // Supabase에서 가족원 목록 불러오기 (의완팀 전용)
      const loadFamilyMembers = async () => {
        try {
          const { data, error } = await supabase
            .from('wordCampus_uiwan_family_members')
            .select('*')
          ...
          if (error) {
            ...
            const storedMembers = localStorage.getItem('campusFamilyMembers_uiwan');
            if (storedMembers) {
          ...
          if (data) {
            ...
            // 로컬 스토리지에도 백업 저장
            localStorage.setItem('campusFamilyMembers_uiwan', JSON.stringify(members));
          }
        } catch (error) {
          console.error('가족원 목록 불러오기 오류:', error);
          // 오류 발생 시 로컬 스토리지에서 백업 데이터 불러오기
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        // 상태 및 로컬 스토리지 업데이트 (의완팀 전용)
        setFamilyMembers(updatedMembers);
        localStorage.setItem('campusFamilyMembers_uiwan', JSON.stringify(updatedMembers));
        localStorage.setItem('campusFamilyRegistered_uiwan', 'true');
  ...
  const handleExportToExcel = () => {
    ...
    const fileName = `김의완_가족원_목록_${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, '0')}${String(today.getDate()).padStart(2, '0')}.xlsx`;
  ...
  return (
    <>
      <Head>
        <title>가족원 등록::김의완 가족 - 포도리더스</title>
        <meta name="description" content="김의완 가족 가족원을 등록하고 관리하세요." />
      </Head>
      ...
      <div className={`min-h-screen bg-gradient-to-b from-emerald-50 to-white ${pretendard.className}`}>
        ...
        <div className="container mx-auto max-w-4xl px-4 py-8">
          ...
          <div className="mb-6 flex items-center justify-between">
            <h1 className="text-3xl font-bold text-emerald-800">가족원 등록</h1>
          ...
          <div className="mb-8 overflow-hidden rounded-2xl bg-white shadow-lg">
            <div className="border-b border-emerald-100 bg-emerald-50 px-6 py-4">
              <h2 className="text-xl font-bold text-emerald-800">새 가족원 등록</h2>
              <p className="mt-1 text-sm text-emerald-600">김의완 가족에 새로운 가족원을 추가합니다.</p>
            </div>
