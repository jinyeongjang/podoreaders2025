...
export default function LocationsUiwanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data) {
        ...
        // 로컬 스토리지에 백업 저장
        localStorage.setItem('wordCampusLocations_uiwan', JSON.stringify(updatedLocations));
  ...
  const confirmDelete = async () => {
    try {
      // Supabase에서 모임장소 삭제
      const { error } = await supabase.from('wordCampus_uiwan_locations').delete().eq('id', locationToDelete);
      ...
      <main className="container mx-auto max-w-2xl px-4 py-2">
        ...
        <div className="mb-4 mt-4">
          <button
            ...
            <FaArrowLeft className="h-4 w-4" /> 김의완 가족 페이지로 돌아가기
          </button>
        </div>
