...
export default function WordCampusMinhwaNoticesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && editingNoticeId) {
        ...
        // 등록 모드
        const { error } = await supabase.from('wordCampus_notices_minhwa').insert({
          title: title.trim(),
          content: content.trim(),
        });
  ...
  const confirmDelete = async () => {
    ...
    try {
      const { error } = await supabase.from('wordCampus_notices_minhwa').delete().eq('id', deletingNotice.id);
