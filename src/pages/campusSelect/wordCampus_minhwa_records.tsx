...
// import campusUsersWordminhwa from '../../data/campusUsersWord_minhwa.json';

// 말씀캠퍼스 민화 가족 전용 QT 기록 타입
...
export default function WordCampusMinhwaRecords() {
  const [records, setRecords] = useState<WordCampus_minhwa_QtRecord[]>([]);
  ...
  const groupedByDate = useMemo(() => {
    const groups: Record<string, WordCampus_minhwa_QtRecord[]> = {};
    filteredRecords.forEach((record) => {
  ...
  const groupRecordsByWeek = useMemo(() => {
    const grouped: Record<string, Record<string, WordCampus_minhwa_QtRecord[]>> = {};
        ...
        (acc, userName) => {
          acc[userName] = Object.entries(grouped[userName])
            ...
            .reduce(
              ...
              {} as Record<string, WordCampus_minhwa_QtRecord[]>,
            );
          return acc;
        },
        {} as Record<string, Record<string, WordCampus_minhwa_QtRecord[]>>,
      );
  }, [filteredRecords]);
  ...
  // 주차별 통계 계산
  const calculateWeekStats = (recs: WordCampus_minhwa_QtRecord[]) => {
    return recs.reduce(
      (acc, record) => ({
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '말씀캠퍼스 노민화 가족 QT 기록');

    const fileName = `말씀캠퍼스 노민화 가족 QT기록_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  return (
    <div
      ...
      <main className="container mx-auto max-w-7xl px-4 py-0">
        <RecordsHeader onExport={exportToExcel} familyName="노민화 가족" />
        ...
        {/* <WordCampusStatsSection
          userList={campusUsersWordminhwa.users.map((u) => u.name)}
          userStats={userStats}
