...
export function useWordCampusUiwan() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const loadLocations = async () => {
        try {
          const { data, error } = await supabase
            .from('wordCampus_uiwan_locations')
            .select('*')
            .order('created_at', { ascending: false })
            .limit(1);
          ...
          if (error) {
            ...
            const savedLocations = localStorage.getItem('wordCampusLocations_uiwan');
            if (savedLocations) {
          ...
          if (data) {
            ...
            localStorage.setItem('wordCampusLocations_uiwan', JSON.stringify(locationData));
          }
        } catch (error) {
          console.error('모임장소 로딩 중 오류 발생:', error);
          const savedLocations = localStorage.getItem('wordCampusLocations_uiwan');
  ...
  const handleRegisterClick = useCallback(() => {
    router.push('/campusSelect/familyRegistration_uiwan');
  }, [router]);

  const handleNoticeClick = useCallback(() => {
    router.push('/campusSelect/wordCampus_uiwan_notices');
  }, [router]);

  const handleLocationClick = useCallback(() => {
    router.push('/campusSelect/wordCampus_uiwan_location');
  }, [router]);

  const handleRecordsClick = useCallback(() => {
    router.push('/campusSelect/wordCampus_uiwan_records');
  }, [router]);
